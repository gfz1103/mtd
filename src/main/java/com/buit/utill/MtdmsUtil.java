...
@Service
public class MtdmsUtil {

    static final Logger logger = LoggerFactory.getLogger(MtdmsUtil.class);
...
//			newKey = newKey.replace("_", "");
    ...
    public static double formatDouble(int number, double data) {
        return MtdmsUtil.getDouble(data, number);
    }
    ...
    /**
     * 日历周类型频次获取上一次执行的日期(包括当天，eg: 每天执行的频次查找到的下次执行日期就是传入的日期)
     *
     * @param rzxzq     日执行周期表达式第一位代表星期日
     * @param startDate 开始查找日期
     */
    public static String getLastExecuteDateByWeek(String rzxzq, String startDate) {
        Date start = DateUtils.parseToDate(startDate);
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeZone(TimeZone.getTimeZone("GMT+8:00"));
        calendar.setTime(start);
        //起始查找位置(所属本周天数减一即为在周期字符串中的下标）
        //eg: 2021-04-25为星期日, 周期为'1000000'(每周日执行)，获取dayOfWeek为1,起始下标为0
        int startIndex = calendar.get(Calendar.DAY_OF_WEEK) - 1;
        for (int step = 0; step < 7; step++) {
            int current = startIndex - step;
            if (current < 0) {
                current = current + 7;
            }
            if (current >= 7) {
            ...
            char c = rzxzq.charAt(current);
            //遇到1则返回行走的步数
            if ('1' == c) {
                calendar.add(Calendar.DATE, 0 - step);
                return DateUtils.formatToDate(calendar.getTime());
            }
        }
        throw BaseException.create("ERROR_ZL_00039");
    }

    /**
     * 日历周类型频次获取下次执行的日期(包括当天，eg: 每天执行的频次查找到的下次执行日期就是传入的日期)
     *
     * @param rzxzq     日执行周期表达式第一位代表星期日
     * @param startDate 开始查找日期
     */
    public static String getNextExecuteDateByWeek(String rzxzq, String startDate) {
        Date start = DateUtils.parseToDate(startDate);
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeZone(TimeZone.getTimeZone("GMT+8:00"));
        calendar.setTime(start);
    ...
    /**
     * 非日历周类型获取上一次执行的日期(不包括当天)
     *
     * @param zxzq        执行周期（执行间隔天数）
     * @param rzxzq       日执行周期表达式（对于日历周来说，第一位代表星期日）
     * @param executeDate 本次执行日期
     */
    public static String getLastExecuteDateByDays(int zxzq, String rzxzq, String executeDate) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeZone(TimeZone.getTimeZone("GMT+8:00"));
        calendar.setTime(DateUtils.parseToDate(executeDate));

        int startIndex = rzxzq.lastIndexOf("1");//本次执行所属的下标
        if (startIndex >= 0) {
            //从该下标往前查找
            for (int step = 1; step <= zxzq; step++) {
                int current = startIndex - step;
                if (current < 0) {
                    current = current + zxzq;
                }
                if (current >= zxzq) {
                    current = current - zxzq;
                }
    ...
    /**
     * 非日历周类型获取下次执行的日期(不包括当天)
     *
     * @param zxzq            执行周期（执行间隔天数）
     * @param rzxzq           日执行周期表达式（对于日历周来说，第一位代表星期日）
     * @param lastExecuteDate 最近一次执行日期
     */
    public static String getNextExecuteDateByDays(int zxzq, String rzxzq, String lastExecuteDate) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeZone(TimeZone.getTimeZone("GMT+8:00"));
        calendar.setTime(DateUtils.parseToDate(lastExecuteDate));

        int startIndex = rzxzq.indexOf("1");//上次执行所属的下标
        if (startIndex >= 0) {
            //从该下标之后开始查找
            for (int step = 1; step <= zxzq; step++) {
                int current = startIndex + step;
